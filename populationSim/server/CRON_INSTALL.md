...
1) Copy the script into place (already created at populationSim/server/scripts/check_population_site.sh)
2) Make it executable (done): chmod +x populationSim/server/scripts/check_population_site.sh
...
Add the following line (runs at minute 0 every hour):
   ...
   0 * * * * /root/clawd/populationSim/server/scripts/check_population_site.sh
